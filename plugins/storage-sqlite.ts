...
import type { Actor, Entity, Plugin, Store } from '../packages/contracts.ts';
...
export class SqliteStore implements Store {
  ...
  saveSession(hash: string, actor: Actor, expires: string) {
    this.db.prepare('INSERT INTO sessions VALUES(?,?,?)').run(hash, JSON.stringify(actor), expires);
  }
  session(hash: string) {
    const row = this.db.prepare('SELECT actor,expires FROM sessions WHERE hash=?').get(hash);
    return row
      ? { actor: JSON.parse(String(row.actor)) as Actor, expires: String(row.expires) }
      : undefined;
  }
