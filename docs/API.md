...
Optional laboratory integration APIs, scoped machine authentication, durable receipt semantics and executable schemas are documented in [INTEGRATIONS.md](INTEGRATIONS.md). Result intake is at `/integrations/:connectorId/results`, outside the clinician `/api` namespace. `202` acknowledges durable receipt, not clinical application. The profile-aware OpenAPI document advertises these routes only when enabled.
