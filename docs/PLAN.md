...
The 2026-09-19 identity/access release implements OIDC validation against a signed test provider, durable assignments/revocation, unit/action/patient policy, protected-record exclusion and manual review (IDENTITY-AND-ACCESS.md). M1 is not complete: real SITHS/HSA integration, an approved clinic access matrix, country-specific patient rights and independent review remain. Prioritize these alongside persistent pilot architecture; local hash verification is not external audit anchoring.
