...
test('care relationship, tenant, role, restriction and proxy expiry all constrain reads', async (t) => {
  ...
  await assert.rejects(
    () => f.clinical.chart({ ...doctor, id: 'unassigned' }, f.patient.id),
    /No active/,
  );
  const proxy = { id: 'guardian', tenant: doctor.tenant, role: 'proxy' as const };
  await assert.rejects(() => f.clinical.chart(proxy, f.patient.id), /No active/);
  await access.grant(doctor, f.patient.id, proxy.id, 'proxy', '2099-01-01T00:00:00Z');
  assert.equal((await f.clinical.chart(proxy, f.patient.id)).length, 2);
  await assert.rejects(
  ...
  await access.block(self, f.patient.id, true);
  await assert.rejects(() => f.clinical.chart(doctor, f.patient.id), /No active/);
  assert.equal((await f.clinical.patients(doctor)).length, 0);
  assert.equal((await f.clinical.chart(self, f.patient.id)).length, 2);
  await access.block(self, f.patient.id, false);
  assert.equal((await f.clinical.chart(doctor, f.patient.id)).length, 2);
  assert.ok(
    f.store.db
...
test('patients do not see draft notes or AI proposals, including in history/export', async (t) => {
  ...
  assert.equal((await f.clinical.chart(self, f.patient.id)).length, 2);
  assert.equal((await f.clinical.history(self, note.id)).length, 0);
  await f.clinical.transition(doctor, note.id, 'sign', 1, {});
  assert.equal((await f.clinical.history(self, note.id)).length, 1);
});
test('HTTP fails closed, ignores spoofed roles, blocks audit and expires/revokes tokens', async (t) => {
  ...
  const expired = await f.runtime.get('identity').issue!(doctor);
