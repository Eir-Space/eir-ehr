...
test('AI proposal requires review, creates only draft, and rejects replay or stale context', async (t) => {
  ...
  assert.equal(f.store.list(doctor.tenant, f.patient.id, 'note').length, 0);
  const accepted = ai.review(doctor, proposal.id, 1, 'accept');
  assert.equal(f.store.get(doctor.tenant, accepted.data.noteId)?.data.status, 'draft');
  assert.throws(() => ai.review(doctor, proposal.id, 1, 'accept'), /already reviewed/);
  ...
  assert.throws(() => ai.review(doctor, stale.id, 1, 'accept'), /context changed/);
  const rejected = ai.review(doctor, stale.id, 1, 'reject');
...
test('AI rejects invented citations and rechecks access after inference', async (t) => {
  ...
  assert.equal(f.store.list(doctor.tenant, f.patient.id, 'proposal').length, 0);
});
